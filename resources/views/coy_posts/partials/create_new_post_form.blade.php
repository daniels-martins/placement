<section>
   <header>
      @if (session('status') === 'job-listing-created-successfully')
      <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 5000)"
         class="text-sm text-gray-600">{{ __('Saved. Operation Successful') }}</p>
      @endif
   ...
   <form method="post" action="{{ route('coy-post.store') }}" class="mt-6 space-y-6">
      ...
      <div>
         ...
         <textarea name="about_us" id="about_us" rows="5" class="mt-1 block w-full">{{ old('about_us') }}</textarea>
      </div>
      ...
      <div>
         ...
         <textarea autocomplete="responsibilities" name="responsibilities" id="responsibilities" rows="10"
            class="mt-1 block w-full">{{ old('responsibilities') }}</textarea>
      ...
      <div>
         ...
         <div>
            ...
            <textarea autocomplete="payment_info" name="payment_info" id="payment_info" rows="10"
               class="mt-1 block w-full">{{ old('payment_info') }}</textarea>
      ...
      <div>
         ...
         <textarea autocomplete="benefits" name="benefits" id="benefits" rows="10"
            class="mt-1 block w-full">{{ old('benefits') }}</textarea>
      ...
      <div>
         ...
         <textarea autocomplete="details" name="details" id="details" rows="10"
            class="mt-1 block w-full">{{ old('details') }}</textarea>
      ...
      <div>
         ...
         <textarea autocomplete="requirements" name="requirements" id="requirements" rows="10"
            class="mt-1 block w-full">{{ old('requirements') }}</textarea>
      ...
      <div>
         ...
         <x-text-input id="coy_website" name="coy_website" type="text" class="mt-1 block w-full"
            :value="old('coy_website')" required autofocus autocomplete="coy_website" />
